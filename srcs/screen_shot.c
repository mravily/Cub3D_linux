...
/*   Updated: 2020/10/17 18:08:36 by mravily          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		write_bmp_header(int fd, int filesize)
{
	...
	tmp = g_engine->config->resolution.x;
	set_int_in_char(bmpfileheader + 18, tmp);
	tmp = g_engine->config->resolution.y;
...
static int		get_color_bmp(int x, int y)
{
	int	color;

	color = *(int*)(g_engine->image->addr + (4
		* (int)g_engine->config->resolution.x *
				((int)g_engine->config->resolution.y - 1 - y)) + (4 * x));
	return (color);
}

static int		write_bmp_data(int file)
{
	int					i;
	int					j;
	int					color;
	...
	i = 0;
	while (i < (int)g_engine->config->resolution.y)
	{
		j = 0;
		while (j < (int)g_engine->config->resolution.x)
		{
			color = get_color_bmp(j, i);
...
int				screen_shot(void)
{
	...
	filesize = 54 + (3 * (int)g_engine->config->resolution.x
